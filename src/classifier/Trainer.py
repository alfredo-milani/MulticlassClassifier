...
class Trainer(AbstractClassifier):
    ...
    def fiverr_k(self):
        dataset = pd.read_csv("/Volumes/Ramdisk/Fiverr/training_set.csv")
        test = pd.read_csv("/Volumes/Ramdisk/Fiverr/test_set.csv")

        dataset['F1'] = dataset['F1'].replace(np.NaN, dataset['F1'].median())
        dataset['F2'] = dataset['F2'].replace(np.NaN, dataset['F2'].median())
        dataset['F3'] = dataset['F3'].replace(np.NaN, dataset['F3'].median())
        dataset['F4'] = dataset['F4'].replace(np.NaN, dataset['F4'].median())
        dataset['F5'] = dataset['F5'].replace(np.NaN, dataset['F5'].median())
        dataset['F6'] = dataset['F6'].replace(np.NaN, dataset['F6'].median())
        dataset['F7'] = dataset['F7'].replace(np.NaN, dataset['F7'].median())
        dataset['F8'] = dataset['F8'].replace(np.NaN, dataset['F8'].median())
        dataset['F9'] = dataset['F9'].replace(np.NaN, dataset['F9'].median())
        dataset['F10'] = dataset['F10'].replace(np.NaN, dataset['F10'].median())
        dataset['F11'] = dataset['F11'].replace(np.NaN, dataset['F11'].median())
        dataset['F12'] = dataset['F12'].replace(np.NaN, dataset['F12'].median())
        dataset['F13'] = dataset['F13'].replace(np.NaN, dataset['F13'].median())
        dataset['F14'] = dataset['F14'].replace(np.NaN, dataset['F14'].median())
        dataset['F15'] = dataset['F15'].replace(np.NaN, dataset['F15'].median())
        dataset['F16'] = dataset['F16'].replace(np.NaN, dataset['F16'].median())
        dataset['F17'] = dataset['F17'].replace(np.NaN, dataset['F17'].median())
        dataset['F18'] = dataset['F18'].replace(np.NaN, dataset['F18'].median())
        dataset['F19'] = dataset['F19'].replace(np.NaN, dataset['F19'].median())
        dataset['F20'] = dataset['F20'].replace(np.NaN, dataset['F20'].median())
        test['F1'] = test['F1'].replace(np.NaN, dataset['F1'].median())
        test['F2'] = test['F2'].replace(np.NaN, dataset['F2'].median())
        test['F3'] = test['F3'].replace(np.NaN, dataset['F3'].median())
        test['F4'] = test['F4'].replace(np.NaN, dataset['F4'].median())
        test['F5'] = test['F5'].replace(np.NaN, dataset['F5'].median())
        test['F6'] = test['F6'].replace(np.NaN, dataset['F6'].median())
        test['F7'] = test['F7'].replace(np.NaN, dataset['F7'].median())
        test['F8'] = test['F8'].replace(np.NaN, dataset['F8'].median())
        test['F9'] = test['F9'].replace(np.NaN, dataset['F9'].median())
        test['F10'] = test['F10'].replace(np.NaN, dataset['F10'].median())
        test['F11'] = test['F11'].replace(np.NaN, dataset['F11'].median())
        test['F12'] = test['F12'].replace(np.NaN, dataset['F12'].median())
        test['F13'] = test['F13'].replace(np.NaN, dataset['F13'].median())
        test['F14'] = test['F14'].replace(np.NaN, dataset['F14'].median())
        test['F15'] = test['F15'].replace(np.NaN, dataset['F15'].median())
        test['F16'] = test['F16'].replace(np.NaN, dataset['F16'].median())
        test['F17'] = test['F17'].replace(np.NaN, dataset['F17'].median())
        test['F18'] = test['F18'].replace(np.NaN, dataset['F18'].median())
        test['F19'] = test['F19'].replace(np.NaN, dataset['F19'].median())
        test['F20'] = test['F20'].replace(np.NaN, dataset['F20'].median())

        # outliers by caping the all columns
        ...
        dataset = cap_data(dataset)
        # test = cap_data(dataset)

        # finding uper and lower quartiles
        Q1 = dataset.quantile(0.25)
        Q3 = dataset.quantile(0.75)
        # IQR stands for Inter Quartile Range, used for dealing with outliers
        IQR = Q3 - Q1
        ...
        y_test = test["CLASS"]
        x_test = prep.normalize(x_test, 'l1')

        # minmax normalization
        x = prep.normalize(x, "l1")

        # oversampling
        from sklearn.preprocessing import LabelEncoder
        # making balance in each class
        y = LabelEncoder().fit_transform(y)
        # size_adjust = {0:2421, 1:3200, 2:3100, 3:2151}  # manually balancing classes
        # oversample = SMOTE(sampling_strategy=size_adjust)
        oversample = SMOTE()
        x, y = oversample.fit_resample(x, y)

        from sklearn.model_selection import train_test_split
        # x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.1, random_state=10)
        # x_train, _, y_train, _ = train_test_split(x, y, test_size=0.1, random_state=10)
        # x_test = test.drop(["CLASS"], axis=1)
        # y_test = test["CLASS"]
        x_train = x
        y_train = y
